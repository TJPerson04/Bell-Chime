...
CHIME_HOURS = [9, 11, 15, 17]  # If you add an hour make sure to add a corresponding minute
CHIME_MINS = [0, 58, 0, 58]  # CHIME_MINS & CHIME_HOURS should be the same length
RESET_HOUR = 23
...
for i in range(len(CHIME_HOURS)):
    ...
    output += str(hour) + ":" + formatTime(min)
...
while True:
    ...
    for i in range(len(CHIME_HOURS)):
        ...
        if currentHour == hour and currentMin == min and not is_played:
            ...
            numRepeat = hour % 12
            if min > 30:  # This is in case the bells start a little before an hour, then the listed hour will be 1 behind
                numRepeat += 1
            if numRepeat == 0:
                numRepeat = 12
